...
EXPORT_PATH = "/home/scraping/algo_scraping/exploration/rakuten/NEWdata_cleaned_rakuten_lag3.csv"
...
def create_ids(df):
    ...
    # Calcul de Price_lag (pour avoir le prix précédent)
    df["Price_lag"] = df.groupby("ReadableID")["Price"].shift(1)
    return df
...
def main():
    ...
    df = df[["Timestamp", "Day", "Seller", "CodeID", "ReadableID", 
             "Price", "Price_lag", "n_true_changes_day", "algo_suspect", "offertype", "shipcost"]].copy()
    


    # Exporter le fichier nettoyé avec les variables lagées
    df.to_csv(EXPORT_PATH, index=False)
    
    # Plots des changements de prix
    #plot_true_price_changes_boxplot_for_top_models(df)
    #plot_cdf_true_price_changes(df)
    #plot_cdf_daily_price_changes(df)
