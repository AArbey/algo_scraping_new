...
CSV_PATH = "/home/scraping/algo_scraping/scraping_cdiscount.csv"
EXPORT_PATH = "/home/scraping/algo_scraping/exploration/cdiscount/data_cleaned_cdiscount.csv"
FIG_DIR = "/home/scraping/algo_scraping/exploration/cdiscount/figures"
os.makedirs(FIG_DIR, exist_ok=True)
os.makedirs(os.path.dirname(EXPORT_PATH), exist_ok=True)

def load_and_clean_data(path):
    df = pd.read_csv(path, 
                    names=["Platform", "Product", "Price", "Seller", "Status", "Rating", "Delivery", "Timestamp"],
                    skiprows=1, 
                    on_bad_lines='skip',
                    sep=',')
...
def generate_visualizations(df):
    ...
    if df["ModelCode"].nunique() > 1:
        ...
        top_models = df["ModelCode"].value_counts().nlargest(10).index
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(FIG_DIR, "price_distribution_products.png"))
        plt.close()
        
    if df["Platform"].nunique() > 1:
        plt.figure(figsize=(14, 8))
        top_platforms = df["Platform"].value_counts().nlargest(10).index  # Les 5 plateformes les plus fréquentes
        sns.boxplot(data=df[df["Platform"].isin(top_platforms)],
                    x="Platform", y="Price", hue="State")
        plt.title("Distribution des prix par plateforme")
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(os.path.join(FIG_DIR, "price_distributions_platforms.png"))
        plt.close()

    # Vérifier s'il y a plusieurs fournisseurs
    if df["Seller"].nunique() > 1:
        plt.figure(figsize=(14, 8))
        top_suppliers = df["Seller"].value_counts().nlargest(10).index  # Les 5 fournisseurs les plus fréquents
        sns.boxplot(data=df[df["Seller"].isin(top_suppliers)],
                    x="Seller", y="Price", hue="State")
        plt.title("Distribution des prix par fournisseur")
        plt.xticks(rotation=45)
        plt.tight_layout()
        plt.savefig(os.path.join(FIG_DIR, "price_distributions_sellers.png"))
        plt.close()
